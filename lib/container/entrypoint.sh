...
# Create config dirs and write config files as root, then fix ownership
...
if [[ "${RUN_AS_USER:-}" == "1" ]] || [[ "${RUN_AS_ROOTISH:-}" == "1" ]]; then
    ...
    # Fix ownership of config files we just wrote as root
    # Only chown specific files + top-level dirs (NOT recursive on .claude — it's a volume with session history)
...
# (if running as root without privilege drop, config files are already written above)
