...
main() {
    ...
    _stale_vols=$(docker volume ls -q | grep -E '^claude-(session|state|cargo|npm|pip)-test-multi-' 2>/dev/null || true)
    ...
    rm -rf "$HOME/.cache/claude-container-test-"* 2>/dev/null || true
